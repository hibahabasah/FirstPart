...
boolean added=false;
	 ...
	 boolean bo=Boolean.parseBoolean(string2);
	   users s = new users(string,int1,bo);
	   obj.add_users(s.getusername(),s.getpassward());
	   user.add(s);
	   status=bo;
    }
	...
	//scenario2 
			...
			public void loginginWithUsernameAndPasswordAnd(String string, String string2, String string3) {
				...
				obj.add_users(us.getusername(), us.getpassward());
			}
...
//scenario log out
				...
				public void iPressLogoutFrom(String string) {
				    status=obj.check_username(string);
				    		if (status)
				    			status=false;
				}
